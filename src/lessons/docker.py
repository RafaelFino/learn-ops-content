...
c.StudentComment(
    "Olá, Fino! Tudo bem! Eu sei mais ou menos o que é virtualização, mas não sei muito bem a diferença entre virtualização e containers.")
...
c.StudentComment(
    "Quais ferramentas são usadas para fazer essa virtualização? Consigo fazer isso no meu PC de casa ou preciso de um super servidor?")
...
c.StudentComment(
    "Pode me ensinar a instalar o Docker no Windows? Quero aprender mais sobre containers!")
...
c.StudentComment(
    "Desculpa, Fino! Eu estava distraído... vou prestar mais atenção agora! Pode me ensinar a instalar o Docker no Linux?")
...
c.StudentComment(
    "Desculpa, Fino! Eu me empolguei com o Docker... pode me explicar o que é um container?")
...
c.StudentComment(
    "Entendi... e o Docker, o que é? E qual a relação dele com os containers?")
...
c.StudentComment(
    "E quem fez esse tal de Docker? Ele é muito famoso? Como ele foi feito?")
...
c.StudentComment(
    "E quais são as vantagens de usar o Docker? Por que ele é tão popular?")
...
c.StudentComment(
    "É verdade que o Docker foi feito usando Golang? O que é Golang?")
...
c.Question(
    "Eu consigo criar um container com o Docker para executar aplicações Windows?")
...
c.StudentComment(
    "Não Fino, consigo executar aplicações feitas para Windows dentro de um container Linux?")
...
c.StudentComment(
    "Entendi... então não posso usar para jogar meu Counter Strike na Steam com docker?")
...
c.StudentComment(
    "Pode me mostrar um exemplo de como criar um container com o Docker?")
...
c.StudentComment(
    "Caramba, que legal! Consigo fazer isso no meu PC de casa? Vou poder brincar de hacker?")
...
c.StudentComment(
    "Obrigado, Fino! Vou começar a brincar com o Docker agora mesmo! Você tem alguma dica para eu começar?")
...
c.StudentComment(
    "Obrigado, Fino! Vou seguir esses passos e instalar o Docker no meu PC! Você é o melhor professor de todos!")
c.Speak("Disponha, meu jovem Aspirante a Ex Pobre... mas onde você vai? não terminamos a aula ainda! Tá com preguiça???")
c.StudentComment(
    "Desculpa, Fino! Eu estava tão empolgado com o Docker que me esqueci da aula... vamos continuar!")
c.Speak("Ficou clara a relação entre Docker, Docker Hub, Containers, Imagens e virtualização?")
c.StudentComment(
    "Sim, Fino! Acho que sim, mas poderia por favor explicar de uma forma bem simples, consolidade e resumida para fixar na minha cabecinha oca?")
...
c.StudentComment(
    "E que outras vantagens usar Docker pode trazer para mim? É verdade que eu consigo automatizar tarefas com Docker?")
...
c.StudentComment(
    "E como eu posso aprender mais sobre Docker? Você tem alguma dica de material de estudo?")
...
c.StudentComment(
    "Entendi... você poderia me mostrar como criar um Dockerfile?")
...
c.Speak("Agora você está rodando um servidor web em um container. Você pode acessar o servidor web no seu navegador em http://localhost:8080 ou com o comando curl.")
c.ShowCommand("curl http://localhost:8080")
c.Speak("Parabéns, pequeno Gafanhoto! Você criou seu primeiro Dockerfile e rodou um servidor web em um container. O que você achou?")
c.StudentComment(
    "Caramba, que legal! Consigo fazer isso no meu PC de casa? Vou poder criar meus próprios servidores web?")
...
c.StudentComment(
    "E como eu vejo quais containers estão rodando no meu PC? Consigo listar todos os containers com Docker?")
...
c.StudentComment(
    "E como eu paro um container que está rodando no meu PC? Consigo parar um container com Docker?")
...
c.StudentComment(
    "E se eu quiser remover uma imagem do Docker? Consigo remover uma imagem com Docker?")
...
c.StudentComment(
    "E se eu quiser executar um comando dentro de um container? Consigo executar um comando com Docker?")
...
c.StudentComment(
    "Ficou meio confuso isso... o container está dentro do meu PC, mas como ele se comunica com ele e para fora dele?")
...
c.StudentComment(
    "E como eu posso configurar um container com Docker? Consigo usar variáveis de ambiente para configurar um container?")
...
c.StudentComment(
    "E se eu quiser mapear um diretório do meu PC para um container? Consigo mapear diretórios com Docker?")
...
c.StudentComment(
    "E se eu quiser mapear um volume do meu PC para um container? Consigo mapear volumes com Docker?")
...
c.StudentComment(
    "E se eu quiser criar um Dockerfile? Consigo criar um Dockerfile com Docker?")
...
c.StudentComment(
    "E se eu quiser criar vários containers com Docker se comunicando, como eu faria isso?")
...
c.StudentComment(
    "Preciso estudar mais isso, parece algo incrível! Obrigado, Fino! Você é o melhor professor de todos!")
