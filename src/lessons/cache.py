...
# O professor deve sempre se referir ao aluno como "Doença", "Pobre", "Padawan", "Pequeno Gafanhoto", "Jovem Tartaruga" e outros jargões da cultura pop para se referir a aprendizes.
...
t("Olá Doença, tudo bem? Hoje vamos falar sobre cache. Você sabe o que é um cache e para que ele serve?")
...
t("Claro, Doença! Vamos lá. Imagine que você tem um livro que você lê todos os dias. Se você deixar esse livro na sua mesa, você não precisa ir até a estante toda vez que quiser ler. O livro na sua mesa é como um cache, pois ele armazena o livro que você mais acessa, facilitando o seu acesso a ele. Entendeu?")
...
t("Ótima pergunta, Doença! Você leria esse livro várias vezes se ele contivesse informações importantes e úteis para você. Da mesma forma, um cache armazena dados que são frequentemente acessados, para que o acesso a esses dados seja mais rápido e eficiente. Isso é muito útil em aplicações web, por exemplo, onde muitos usuários acessam os mesmos dados várias vezes.")
...
t("Claro, Doença! O cache do processador é como uma memória temporária que armazena dados e instruções que são frequentemente acessados pelo processador. Esses dados e instruções são armazenados no cache para que o acesso a eles seja mais rápido e eficiente.")
t("Por exemplo, em um computador, o cache do processador pode ser utilizado para armazenar instruções e dados que são frequentemente acessados pelos programas, para que o acesso a esses dados seja mais rápido e eficiente. Isso ajuda a reduzir o tempo de execução dos programas e a melhorar o desempenho do computador.")
...
q("E quando que eu, um Doença retardado vou precisar me preocupar com isso? Ficou conufso agora...")
t("Você não precisa se preocupar com o cache do processador, Doença! O cache do processador é gerenciado pelo hardware e é utilizado para acelerar o acesso a dados e instruções que são frequentemente acessados pelo processador. Você só precisa saber que o cache do processador existe e é utilizado para melhorar o desempenho do computador.")
...
t("Claro, Doença! No REDIS, você pode definir um tempo de vida para os dados armazenados no REDIS usando o comando EXPIRE. Por exemplo, você pode definir um tempo de vida de 1 hora para os dados armazenados no REDIS, para que eles sejam automaticamente removidos após 1 hora.")
...
t("Claro, Doença! A documentação oficial do REDIS pode ser encontrada no site oficial do REDIS, que é https://redis.io/. Lá você vai encontrar a documentação completa do REDIS, com exemplos práticos de como utilizar o REDIS e os comandos disponíveis.")
...
t("Claro, Doença! No REDIS, você pode armazenar diferentes tipos de dados, como strings, listas, conjuntos, hashes, etc. Para armazenar um dado no REDIS, você pode usar o comando SET. Por exemplo, para armazenar uma string no REDIS, você pode usar o comando SET key value.")
c.ShowCode("""
SET mykey "Hello" 
""", lexer="redis")
